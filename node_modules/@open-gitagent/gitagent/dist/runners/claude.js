...
export function runWithClaude(agentDir, manifest, options = {}) {
 ...
 */
function collectAllowedTools(agentDir) {
    const tools = new Set();
 ...
 */
function buildSubagentConfig(agentDir, manifest) {
    if (!manifest.agents)
        return null;
    const agents = [];
 ...
 */
function collectExtraDirs(agentDir) {
    const dirs = [];
 ...
 * }
 */
function buildHooksSettings(agentDir) {
    const hooksPath = join(agentDir, 'hooks', 'hooks.yaml');
    if (!existsSync(hooksPath))
        return null;
    try {
        const content = readFileSync(hooksPath, 'utf-8');
        const hooksConfig = yaml.load(content);
        ...
        if (Object.keys(ccHooks).length === 0)
            return null;
