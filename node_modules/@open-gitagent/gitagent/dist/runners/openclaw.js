...
export function runWithOpenClaw(agentDir, manifest, options = {}) {
    ensureOpenClawAuth();
    ...
    if (hasSubAgents) {
        ...
        if (agents.main && typeof agents.main === 'object') {
            agents.main.workspace = mainWorkspace;
        }
        for (const sub of exp.subAgents) {
            if (agents[sub.name] && typeof agents[sub.name] === 'object') {
                agents[sub.name].workspace = join(workspaceDir, `workspace-${sub.name}`);
            }
        }
    }
    else {
    ...
    const args = ['agent', '--local', '--session-id', sessionId, '--message', options.prompt];
