...
import { success, error, info, heading, divider } from '../utils/format.js';
...
export const registryCommand = new Command('registry')
    ...
    try {
        ...
        error(`Failed to load agent.yaml: ${e.message}`);
        process.exit(1);
    }
    if (!manifest.name) {
    ...
    try {
        const gitOpts = { cwd: tmpDir, stdio: 'pipe' };
        ...
        error(`Failed to push: ${e.message}`);
        cleanup(tmpDir);
        process.exit(1);
    }
    // ── Step 10: Create PR ──
    ...
    let prUrl;
    try {
        prUrl = execSync(`gh pr create --repo ${REGISTRY_REPO} --head ${ghUser}:${branchName} --title "Add ${manifest.name} agent" --body "$(cat <<'PREOF'
## Agent Submission
...
PREOF
)"`, { encoding: 'utf-8', cwd: tmpDir, stdio: ['pipe', 'pipe', 'pipe'] }).trim();
    }
    catch (e) {
...
function buildReadme(manifest, repoUrl) {
