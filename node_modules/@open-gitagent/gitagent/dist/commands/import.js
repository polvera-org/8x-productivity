...
function importFromClaude(sourcePath, targetDir) {
...
function importFromCursor(sourcePath, targetDir) {
    const sourceDir = resolve(sourcePath);
    const dirName = basename(sourceDir);
    ...
    if (mdcRules.length > 0) {
        ...
        if (globalRules.length > 0) {
            const soulParts = [`# Soul — imported from Cursor rules\n`];
...
function importFromCrewAI(sourcePath, targetDir) {
    ...
    try {
        const crewConfig = yaml.load(content);
        // Extract first agent definition
        const agents = crewConfig.agents;
        if (!agents) {
        ...
        let soulContent = '# Soul\n\n';
        if (agentDef.role)
            soulContent += `## Core Identity\n${agentDef.role}\n\n`;
        if (agentDef.backstory)
            soulContent += `## Background\n${agentDef.backstory}\n\n`;
        if (agentDef.goal)
            soulContent += `## Purpose\n${agentDef.goal}\n\n`;
        writeFileSync(join(targetDir, 'SOUL.md'), soulContent, 'utf-8');
        success('Created SOUL.md');
        ...
        throw new Error(`Failed to parse CrewAI config: ${e.message}`);
    }
}
function importFromCodex(sourcePath, targetDir) {
    const sourceDir = resolve(sourcePath);
    // Codex CLI uses:
    ...
    const rawModel = config.model || undefined;
    const agentYaml = {
        spec_version: '0.1.0',
        name: dirName.toLowerCase().replace(/[^a-z0-9-]/g, '-'),
        version: '0.1.0',
...
function importFromOpenCode(sourcePath, targetDir) {
    const sourceDir = resolve(sourcePath);
...
function importFromGemini(sourcePath, targetDir) {
    const sourceDir = resolve(sourcePath);
    // Look for GEMINI.md
